...
export function EventDefitnitionIcon({ variant, filled, stroke, }) {
    const cx = nodeSvgProps.x + nodeSvgProps.width / 2;
    const cy = nodeSvgProps.y + nodeSvgProps.width / 2;
    const r = nodeSvgProps.width / 2;
    return (_jsx(RoundSvg, { children: _jsx(EventVariantSymbolSvg, { variant: variant !== null && variant !== void 0 ? variant : "none", strokeWidth: 16, filled: filled, stroke: stroke, x: nodeSvgProps.x, y: nodeSvgProps.x, cx: cx, cy: cy, innerCircleRadius: r - 5, outerCircleRadius: r }) }));
}
...
export function TaskIcon({ variant }) {
    return (_jsx(RoundSvg, { children: _jsx(TaskNodeSvg, { ...nodeSvgProps, variant: variant !== null && variant !== void 0 ? variant : "none" }) }));
}
...
export function GatewayIcon({ variant }) {
    return (_jsx(RoundSvg, { children: _jsx(GatewayNodeSvg, { ...nodeSvgProps, width: 200, height: 200, variant: variant !== null && variant !== void 0 ? variant : "none" }) }));
}
