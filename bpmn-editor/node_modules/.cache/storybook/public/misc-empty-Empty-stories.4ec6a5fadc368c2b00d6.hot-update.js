...
function LinkEventSymbolSvg(_ref10) {
  ...
  const padding = paddingFactor * (outerCircleRadius - innerCircleRadius);
  const hx = innerCircleRadius - padding;
  const hy = innerCircleRadius - padding;
  const points = [{
    x: 392.670,
  ...
  const adjustedPoints = points.map(point => ({
    x: cx + (point.x - cx) * (hx / innerCircleRadius),
    y: cy + (point.y - cy) * (hy / innerCircleRadius)
  }));
