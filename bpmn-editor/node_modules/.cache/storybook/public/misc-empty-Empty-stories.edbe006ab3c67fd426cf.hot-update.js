...
function SignalEventSymbolSvg(_ref6) {
  ...
  const farXPoint = 10;
  const closeXPoint = 7.5;
  const highYPoint = 10;
  const lowYPoint = -5;
  const leftUpperLeftPoint = {
    x: cx - innerCircleRadius / 2 - farXPoint,
    y: cy - innerCircleRadius / 2 + lowYPoint
  };
  const rightUpperLeftPoint = {
    x: cx - innerCircleRadius / 2 + closeXPoint,
    y: cy - innerCircleRadius / 2 - highYPoint
  };
